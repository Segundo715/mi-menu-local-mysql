...
**Stack:** Next.js 16 (App Router, webpack) · React 19 · Tailwind CSS 4 · TypeScript · **MySQL** (`mysql2/promise`) · Supabase Storage (solo imágenes)
...
### Base de datos — MySQL es la fuente de verdad

Toda la persistencia de datos pasa por MySQL vía `lib/mysql.ts`, que expone `query<T>(sql, params)` sobre un **pool cacheado en `global`**:
...
Esto es intencional y **no se debe quitar**: sin el cacheo, el hot-reload de `next dev` re-ejecuta el módulo en cada guardado y crea 10 conexiones nuevas sin cerrar las anteriores — agota `max_connections` del servidor MySQL en pocos minutos de desarrollo activo (`Error: Too many connections`). Si eso ocurre, buscar procesos `node` huérfanos con conexiones abiertas al puerto 3306 antes de asumir que el servicio está caído.

Cada dominio tiene su módulo `lib/*Db.ts` que expone funciones async devolviendo tipos camelCase mediante un mapper `toX(row)` (columnas de BD en snake_case):

| Módulo | Tabla | Notas |
|---|---|---|
| `lib/menuDb.ts` | `menu_items` | CRUD, contador `likes`, flag `available` |
| `lib/ordersDb.ts` | `orders` | `status`: pending → preparing → ready → delivered. `items` es JSON, sin FK a `menu_items` |
| `lib/loyaltyDb.ts` | `loyalty_cards` | Modelo de fidelización activo. `card_type` referencia un `id` dentro de `settings.reward_categories` (JSON), sin FK real |
| `lib/reviewDb.ts` | `reviews` | `bad = rating <= 3` (dispara email), `published = rating >= 4` — calculado en código, no en BD |
...
**Esquema completo verificado** (columnas, tipos, `SHOW CREATE TABLE` real): ver `mysql_setup.sql`. Ejecutarlo contra la BD es idempotente (`CREATE TABLE IF NOT EXISTS`).

**Tablas que existen en la BD pero ya no tienen código**: `recipes`, `tv_slides`, `tables`, `inventory`, `birthday_registrations`. Se conservan con sus datos por si se recuperan, pero ningún `lib/*Db.ts` ni ruta API las toca — no reintroducir referencias a ellas sin antes recrear el módulo correspondiente.

**`lib/mysql.ts`:** además del pool cacheado, expone `toIso(value)` (convierte `DATETIME` a ISO string) y `parseJsonColumn(value, fallback)` (parsea columnas JSON que a veces llegan como string según la versión del driver).
...
- `MYSQL_HOST`, `MYSQL_PORT`, `MYSQL_USER`, `MYSQL_PASSWORD`, `MYSQL_DATABASE` — **requeridas**, conexión a la BD real
...
- **2026-08-14 — Reducción de alcance:** se eliminaron recetario, TV, reservaciones, analíticas, paneles completos de empleado/resta3 (excepto `/menu` de cada uno), y las 5 tablas de BD que ya no tenían código. Se agregó `/admin/tarjetas` y `/admin/sellar` al panel admin para no perder la gestión de fidelización al quitar los paneles de empleado/resta3. Se corrigió un leak de conexiones MySQL en dev (pool no sobrevivía el hot-reload — ver sección de MySQL arriba) y se creó la tabla `sa_tickets` que faltaba en la BD real (rompía "Reportar problema" silenciosamente).
- **Dependencias npm huérfanas retiradas:** `konva`, `react-konva` (eran del floor-plan de reservaciones), `lottie-react` (animaciones de TV), `pg` (nunca se usó, este proyecto es 100% MySQL).
...
- **Agregar un campo persistido** → actualizar el mapper `toX(row)`, los payloads de inserción/actualización y la `interface` en `lib/*Db.ts`. Si es una tabla nueva, incluir `restaurant_id VARCHAR(100) DEFAULT 'default'` y agregarla a `mysql_setup.sql`.
- **Nunca quitar el cacheo en `global` de `lib/mysql.ts`** — ver sección de MySQL arriba.
