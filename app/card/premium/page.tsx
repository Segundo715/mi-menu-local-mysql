...
export default function CardPremiumPage() {
  ...
  const [cfg, setCfg] = useState<PremiumConfig>(() => ({
    ...DEFAULT_PREMIUM,
    logo: brand.logo || DEFAULT_PREMIUM.logo,
    logoColor: brand.logoColor || DEFAULT_PREMIUM.logoColor,
    brandText: brand.name || DEFAULT_PREMIUM.brandText,
  }))
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const brandName = nameRes?.value || brand.name || DEFAULT_PREMIUM.brandText
      const brandLogoUrl = logoRes?.value || pLogoRes?.value || brand.logo || DEFAULT_PREMIUM.logo
